...
import { createFooterFactory, renderFooterLine, type FooterTheme } from "../src/footer.ts";
...
describe("createFooterFactory", () => {
  it("renders current state and requests refresh on branch changes", () => {
    ...
    const component = factory(
      { requestRender: () => { refreshes += 1; } },
      theme,
      {
        getExtensionStatuses: () => statuses,
        onBranchChange: (listener) => { branchListener = listener; return () => { branchListener = undefined; }; },
      },
    );
