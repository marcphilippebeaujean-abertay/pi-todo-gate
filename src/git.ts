...
export const spawnExec: Exec = (command, args, options = {}) => new Promise((resolveResult) => {
...
export async function inspectWorktree(exec: Exec, cwd: string): Promise<WorktreeInfo> {
  ...
  const root = rootResult.code === 0 && rootResult.stdout.trim() ? resolve(rootResult.stdout.trim()) : null;
...
export async function findOpenPr(exec: Exec, cwd: string, branch: string): Promise<OpenPrInfo> {
  const result = await exec("gh", ["pr", "list", "--head", branch, "--state", "open", "--json", "url,state", "--limit", "1"], { cwd });
  ...
  try {
    ...
    const state = row.state === "OPEN" || row.state === "CLOSED" || row.state === "MERGED" ? row.state : "UNKNOWN";
    return { url, state };
  } catch {
    return { url: null, state: "UNKNOWN" };
  }
...
export function mergeCommand(command: string): { kind: "git" | "gh"; args: string[] } | null {
  for (const segment of shellSegments(command)) {
    ...
    if (words.length >= 2 && executableName(words[0]) === "git" && words[1] === "merge") {
      return { kind: "git", args: words.slice(2) };
    }
    if (words.length >= 3 && executableName(words[0]) === "gh" && words[1] === "pr" && words[2] === "merge") {
...
async function queryPinnedHead(exec: Exec, cwd: string, prUrl: string): Promise<string | null> {
  const result = await exec("gh", ["pr", "view", prUrl, "--json", "headRefName"], { cwd });
  if (result.code !== 0) return null;
  try {
    const data: unknown = JSON.parse(result.stdout);
    return typeof data === "object" && data !== null && typeof (data as { headRefName?: unknown }).headRefName === "string"
...
async function queryCurrentPr(exec: Exec, cwd: string, target: string): Promise<{ url: string; headRefName: string } | null> {
  const result = await exec("gh", ["pr", "view", target, "--json", "url,headRefName"], { cwd });
  if (result.code !== 0) return null;
  try {
    const data: unknown = JSON.parse(result.stdout);
    if (typeof data !== "object" || data === null) return null;
    const row = data as { url?: unknown; headRefName?: unknown };
    if (typeof row.url !== "string" || typeof row.headRefName !== "string") return null;
...
export async function matchesPinnedPr(exec: Exec, cwd: string, command: string, prUrl: string): Promise<boolean> {
  ...
  if (parsed.kind === "gh") {
    ...
    if (currentPr === null || normalizedUrl(currentPr.url) !== pinned) return false;
  ...
  return head !== null && (targets[0] === head || targets[0] === `refs/heads/${head}`);
}
