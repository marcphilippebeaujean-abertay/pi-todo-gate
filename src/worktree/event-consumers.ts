...
export class Worktree implements WorktreeConsumer {
	...
	removeWorktree(options: { force: boolean }): Promise<ExitActionResult> {
		...
		return this.executeCleanup(context, worktree, options.force);
	}
	...
	private async executeCleanup(
		...
		return this.cleanupNow(context, worktree, force, CLEANUP_SUCCESS);
	}
	...
	private async cleanupNow(
		...
		const isCurrentAfterCleanup = isCurrentWorktree(this.baseline, worktree);
