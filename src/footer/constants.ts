import type { FooterType } from "./events.ts";

export const FOOTER_STATE_TYPE = "pi-todo-gate-footer-state";
export const FOOTER_STATUS_KEY = "pi-todo-gate-footer";
export const FOOTER_HERDR_TYPE = {
	id: "pi-todo-gate-herdr",
	name: "Herdr",
} as const satisfies FooterType;
export const FOOTER_PR_TYPE = {
	id: "pi-todo-gate-pr",
	name: "PR Link",
} as const satisfies FooterType;
export const FOOTER_TASK_TYPE = {
	id: "pi-todo-gate-task",
	name: "Todoist Task",
} as const satisfies FooterType;
export const FOOTER_HERDR_VALUE = "working";
export const FOOTER_CUSTOM_ENTRY_TYPE = "custom";
export const FOOTER_STATE_LABEL = "footer state";
export const FOOTER_FOOTERS_LABEL = "footer state footers";
export const FOOTER_PERSISTED_LABEL = "persisted footer";
export const FOOTER_TYPE_FIELD = "footerType";
export const FOOTER_LOADING_FIELD = "isLoading";
export const FOOTER_TEXT_FIELD = "text";
export const FOOTER_CURRENT_VALUE_FIELD = "currentValue";
export const FOOTER_SPINNER_INTERVAL_MS = 100;
export const FOOTER_SPINNER_FRAMES = [
	"⠋",
	"⠙",
	"⠹",
	"⠸",
	"⠼",
	"⠴",
	"⠦",
	"⠧",
	"⠇",
	"⠏",
] as const;
export const FOOTER_DIM = "dim";
export const FOOTER_STATUS_SEPARATOR = " | ";
export const FOOTER_NONE_TEXT = "none";
export const FOOTER_ACCENT_COLOR = "accent";
export const FOOTER_TEXT_COLOR = "text";
export const FOOTER_NO_PR_LABEL = "PR: none";
export const FOOTER_HTTPS_PROTOCOL = "https:";
export const FOOTER_OPEN_TASK_LABEL = "open";
export const FOOTER_TODOIST_TASK_LABEL = "Todoist Task: ";
export const FOOTER_TASK_NONE_LABEL = "Todoist Task: none";
export const FOOTER_HTTP_PROTOCOL = "http:";
