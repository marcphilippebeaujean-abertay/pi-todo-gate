...
function ensureFileBacked(path: string): void {
  ...
  if (scope === "off" || scope === "memory") {
    throw new Error("Pi task store is unavailable in the configured memory/off scope");
  }
...
function normalizeTask(value: unknown): PiTask {
  if (typeof value !== "object" || value === null || Array.isArray(value)) throw new Error("invalid Pi task store task");
  const data = value as Partial<PiTask>;
  if (typeof data.id !== "string" || typeof data.subject !== "string" || typeof data.description !== "string") {
    throw new Error("invalid Pi task store task");
  }
  const status = data.status === "pending" || data.status === "in_progress" || data.status === "completed" ? data.status : "pending";
  const now = Date.now();
  return {
    id: data.id,
    ...
    blocks: Array.isArray(data.blocks) ? data.blocks.filter((id): id is string => typeof id === "string") : [],
    blockedBy: Array.isArray(data.blockedBy) ? data.blockedBy.filter((id): id is string => typeof id === "string") : [],
...
export async function readPiTaskStore(path: string): Promise<PiTaskStoreData | null> {
  ...
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error("invalid Pi task store");
  ...
  const nextId = typeof data.nextId === "number" && Number.isInteger(data.nextId) && data.nextId > maxId ? data.nextId : maxId + 1;
  return { nextId, tasks };
}
...
export async function writePiTaskStore(path: string, data: PiTaskStoreData): Promise<void> {
  ...
  try {
    ...
    try { await unlink(temporary); } catch { /* preserve the original rename error */ }
    throw error;
  }
}
...
function metadataLines(task: PiTask): string[] {
  ...
  if (task.blocks.length) lines.push(`${PRIVATE_PREFIX}blocks=${task.blocks.join(",")} -->`);
  if (task.blockedBy.length) lines.push(`${PRIVATE_PREFIX}blockedBy=${task.blockedBy.join(",")} -->`);
  return lines;
}

function withoutPrivateLines(description: string): string {
  return description.split(/\r?\n/).filter((line) => !line.trim().startsWith(PRIVATE_PREFIX)).join("\n").trim();
}
...
function parseStatus(content: string): { status: PiTask["status"]; subject: string } {
  const marker = content.match(/^\[([ x~])\]\s*(.*)$/i);
  if (!marker) return { status: "pending", subject: content };
  const status = marker[1].toLowerCase() === "x" ? "completed" : marker[1] === "~" ? "in_progress" : "pending";
  return { status, subject: marker[2] };
}

function markerData(description: string): { clean: string; values: Record<string, string> } {
...
export function todoistSubtasksToPiTasks(children: readonly TodoistChild[]): PiTaskStoreData {
  ...
  for (const child of flattened(children)) {
    ...
    if (Number.isInteger(numericId) && numericId >= nextId) nextId = numericId + 1;
    const blocks = metadata.values.blocks ? metadata.values.blocks.split(",").filter(Boolean) : [];
    const blockedBy = metadata.values.blockedBy ? metadata.values.blockedBy.split(",").filter(Boolean) : [];
    tasks.push({
      id,
