...
function isWorkState(value: unknown): value is WorkState {
  ...
  return STATE_KEYS.every((key) => value[key] === undefined || typeof value[key] === "string");
}

function stateFromEntry(entry: unknown): WorkState | null {
  if (!isRecord(entry) || entry.type !== "custom" || entry.customType !== "pi-todo-gate-state") return null;
