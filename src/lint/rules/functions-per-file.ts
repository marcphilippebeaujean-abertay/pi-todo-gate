...
export const functionsPerFile: LintRule = ({
	...
	const canonicalFacet =
		/\/src\/(pr|todoist|herdr|worktree|prompt-queue|footer)\/(commands|constants|data|event-consumers|event-publishers|user-prompts|notifications|module|module-state|footer-rendering|parsing|internal-state|claim-worker-result|tab-validation|runtime|git|client)\.ts$|\/src\/(event-consumer|main)\.ts$/.test(
			sourceFile.fileName,
		);
