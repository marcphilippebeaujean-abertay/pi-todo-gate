...
An `if` condition must be a boolean identifier, optionally negated. Comparisons, arithmetic, calls, logical combinations, conditional expressions, and truthiness checks on non-boolean values must first be assigned to a local boolean with an intent-revealing name.
...
The TypeScript type checker determines whether identifier conditions are boolean. The rule does not enforce a naming prefix because intent naming is semantic and cannot be reliably inferred; its diagnostic instructs the author to choose a descriptive boolean name.
...
- string literals that fail and direct `const` definitions that pass;
